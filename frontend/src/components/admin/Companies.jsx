...
export default function Companies() {
  ...
  const fetchCompanies = async () => {
    const res = await fetch("https://careerplatform-xu14.onrender.com/admin/companies");
    const data = await res.json();
    if (data.success) setCompanies(data.companies);
  };

  const updateStatus = async (id, status) => {
    const res = await fetch(`https://careerplatform-xu14.onrender.com/admin/companies/${id}`, {
      method: "PATCH",
  ...
  const deleteCompany = async (id) => {
    const res = await fetch(`https://careerplatform-xu14.onrender.com/admin/companies/${id}`, { method: "DELETE" });
    const data = await res.json();
    if (data.success) fetchCompanies();
  };
