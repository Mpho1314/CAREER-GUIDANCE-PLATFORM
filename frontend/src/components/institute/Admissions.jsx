  ...
  useEffect(() => {
    ...
    const fetchCoursesWithApplications = async () => {
      ...
      try {
        ...
        const resCourses = await fetch(`https://careerplatform-o67g.onrender.com/institute/${user.institutionId}/courses`);
        ...
        const coursesWithApplications = await Promise.all(
          dataCourses.courses.map(async (course) => {
            const resApps = await fetch(`https://careerplatform-o67g.onrender.com/institute/${user.institutionId}/applications`);
  ...
  const publishApplication = async (applicationId) => {
    try {
      const res = await fetch(
        `https://careerplatform-o67g.onrender.com/institute/${user.institutionId}/admissions/application/${applicationId}`,
        { method: "PATCH" }
      );
