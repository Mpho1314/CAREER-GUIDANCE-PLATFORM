  ...
  useEffect(() => {
    ...
    const fetchCoursesWithApplications = async () => {
      ...
      try {
        ...
        const resCourses = await fetch(`http://localhost:5000/institute/${user.institutionId}/courses`);
        ...
        const coursesWithApplications = await Promise.all(
          dataCourses.courses.map(async (course) => {
            const resApps = await fetch(`http://localhost:5000/institute/${user.institutionId}/applications`);
  ...
  const publishApplication = async (applicationId) => {
    try {
      const res = await fetch(
        `http://localhost:5000/institute/${user.institutionId}/admissions/application/${applicationId}`,
        { method: "PATCH" }
      );
