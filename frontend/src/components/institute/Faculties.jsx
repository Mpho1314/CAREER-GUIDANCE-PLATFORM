...
const Faculties = () => {
  ...
  const fetchFaculties = async () => {
    try {
      const res = await fetch(`http://localhost:5000/institute/${instituteId}/faculties`);
  ...
  const deleteFaculty = async (facultyId) => {
    try {
      const res = await fetch(`http://localhost:5000/institute/${instituteId}/faculties/${facultyId}`, {
        method: "DELETE",
      });
      const data = await res.json();
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/institute/${instituteId}/faculties/${editingId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
